...
import { db } from "@no-stack/db/client";
...
export const createTRPCContext = async (opts: {
  ...
  supabaseAdminServerClient: SupabaseClient;
}) => {
  const supabase = opts.supabase;
  const supabaseAdminServerClient = opts.supabaseAdminServerClient;
  ...
  return {
    user: user.data.user,
    db,
    supabaseBrowserClient: supabase,
